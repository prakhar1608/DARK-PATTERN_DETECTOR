...
export class PopupFooter extends LitElement {
    ...
    render() {
        return html`
        <div>
            ${brw.i18n.getMessage("textMoreInformation")}: <a href="https://dapde.de/" target="_blank">*</a>.
        </div>
      `;
    }
}
...
document.getElementById("scrapeButton").addEventListener("click", () => {
    chrome.tabs.executeScript({
        code: `(${scrapeVisibleText})();`
    }, (results) => {
        const visibleText = results[0];
        downloadTextFile("visibleText.txt", visibleText);
    });
});
