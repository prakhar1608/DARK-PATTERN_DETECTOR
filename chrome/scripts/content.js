...
// Create a debounced version of the patternHighlighting function with a 500ms delay
const debouncedPatternHighlighting = debounce(patternHighlighting, 500);
...
async function patternHighlighting(waitForChanges = false) {
    ...
    let domCopyA = document.querySelector('main') ? document.querySelector('main').cloneNode(true) : document.body.cloneNode(true);
    ...
    let domCopyB = document.querySelector('main') ? document.querySelector('main').cloneNode(true) : document.body.cloneNode(true);
   ...
   // Set up MutationObserver with the debounced function
...
// Listen for changes and trigger the debounced pattern highlighting
const mutationCallback = async () => {
    await debouncedPatternHighlighting(true); // Use the debounced function
};

// Set up MutationObserver to listen for changes in the DOM
observer.observe(document.body, {
    subtree: true,
    childList: true,
    attributes: true,
    ...
    const observer = new MutationObserver(async function (mutationsList) {
        for (const mutation of mutationsList) {
            if (mutation.type === 'childList' || mutation.type === 'subtree') {
                await patternHighlighting(true); // Re-scan new elements
            }
        }
    });
    observer.observe(document.body, {
...
function sendResults() {
    ...
    setTimeout(() => {
        sendMessageWithRetry(getPatternsResults());
    }, 1000);  // Delay by 1 second
    
    function sendMessageWithRetry(results) {
        
            brw.runtime.sendMessage(results, function(response) {
                if (brw.runtime.lastError) {
                    console.error("Runtime error:", brw.runtime.lastError.message);
                }
...
//             textArray.push(node.nodeValue.trim());
//         }
...
// This code should be in `feedback.js` or in the main JavaScript file.
document.getElementById("reportDarkPatternButton").addEventListener("click", openFeedbackForm);